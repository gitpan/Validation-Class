...
allowing you to develop your validation classes an models to become 
one-on-the-same.

The Validation::Class object system provides automatic generation of attributes 
accessors based on field names, a new method for creating instances of your 
class and methods via the method keyword which creates self-validating routines 
(sorta like method signatures). 
...
validation and error handling while remaining fast and extensible due to its 
...
things I'd just like to be available without worrying too much about how its setup.
