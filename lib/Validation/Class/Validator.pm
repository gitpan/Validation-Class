...
has 'directives' => (
    ...
    lazy    => 1,
    default => sub {
        return $_[0]->can('config') ? $_[0]->config->{DIRECTIVES} : {};
    }
);
...
has 'filters' => (
    ...
    lazy    => 1,
    default => sub {
        return $_[0]->can('config') ? $_[0]->config->{FILTERS} : {};
    }
);
...
has 'mixins' => (
    ...
    lazy    => 1,
    default => sub {
        return $_[0]->can('config') ? $_[0]->config->{MIXINS} : {};
    }
);
...
has plugins => (
    ...
    lazy    => 1,
    default => sub {
        return $_[0]->can('config') ? $_[0]->config->{PLUGINS} : {};
    }
);
...
sub sanitize {
    ...
    foreach my $field (keys %{$self->fields}) {
        ...
        if (defined $f->{alias}) {
            my $aliases =
              "ARRAY" eq ref $f->{alias} ? $f->{alias} : [$f->{alias}];
...
sub use_validator {
    ...
    my $name = $this->{label} ? $this->{label} : $field;
    my $value = $this->{value};
            ...
            if ($self->directives->{$key}) {
                if ($self->directives->{$key}->{validator}) {
                    if ("CODE" eq ref $self->directives->{$key}->{validator}) {
                        ...
                        # validate
                        my $result =
                          $self->directives->{$key}->{validator}
                          ->($this->{$key}, $value, $this, $self);
        ...
        if (defined $self->types->{field}->{$key}) {
            ...
            if ($self->types->{field}->{$key}->{multi}) {
                ...
                if ("ARRAY" eq ref $field->{$key}) {
                    ...
                    @values = @{$field->{$key}};
                    push @values, "ARRAY" eq ref $value ? @{$value} : $value;
